...
void help(const char *exename)
{
    ...
    puts(" --b <bytes_per_string> read byte (default), word, int (AB/ABCD/89ABCDEF)");
    puts(" --w <wrap_line> wrap line after specified number of chars (default: 70)");
    puts(" -x write only hex characters to file (no  and comma/space)");
    ...
    else
    {
        switch(bytes_per_string)
        {
            case 1:
                ...
                for(i=0; i<lSize-1; i++)
                {
                    fprintf(fileOut,"%02X%s",buffer[i],separator);
                    chars_in_line += stringlength;
                    if(chars_in_line >= wrap_line)
                    {
                        fprintf(fileOut, "\n");
            ...
            case 2:
                ...
                for(i=0; i<lSize-2; i+=2)
                {
                    fprintf(fileOut,"%04X%s",(unsigned int)(buffer[i]*256 +
            ...
            case 4:
                ...
                for(i=0; i<lSize-4; i+=4)
                {
                    fprintf(fileOut,"%08X%s",(unsigned int)(buffer[i]*16777216 +
                        buffer[i+1]*65536 + buffer[i+2]*256 +
